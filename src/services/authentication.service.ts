import z from "zod";
import { Register_Request, Login_Request } from "../dtos/authentication.request.js";
// dependencies
...
export class AuthenticationService {
  ...
  register = async (userData: z.infer<typeof Register_Request>) => {
    ...
    userData.hash_password = await this.password_service.hash_password(userData.hash_password);
  ...
  login = async (userData: z.infer<typeof Login_Request>) => {
    ...
    const isMatch = await this.password_service.compare_password(userData.password, user.hash_password);
    if(!isMatch) {
      throw new Error("Email or Password is incorrect");
    }
